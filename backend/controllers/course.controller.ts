...
export const editCourse = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // If thumbnail is provided and it's NOT a URL (not starting with 'https'), upload new
      ...
      // If thumbnail is provided and IS a URL (starts with 'https'), keep old cloudinary info
      if (thumbnail && thumbnail.startsWith("https")) {
        data.thumbnail = {
      ...
      // If no thumbnail provided in update data, keep existing thumbnail unchanged
      if (!thumbnail) {
        data.thumbnail = {
          public_id: courseData.thumbnail?.public_id,
          url: courseData.thumbnail?.url,
...
export const getSingleCourse = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Helper: Remove sensitive fields from course data
      const sanitizeCourse = (course: any) => {
        if (!course?.courseData) return course;
        return {
          ...course.toObject?.() || course, // ensure plain object
      ...
      // Always include purchased count in response
      const sanitizedCourse = sanitizeCourse(course);

      // Cache sanitized data
      await redis.set(courseId, JSON.stringify(sanitizedCourse), "EX", 604800); // 7 days
...
// Get all courses --- without purchasing (public view)
export const getAllCourses = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await redis.set(cacheKey, JSON.stringify(courses), "EX", 600);
